...
run_name = f'in21k-swinssr_bs4_lr{lr}_{dataset}_{sub_dataset}'

model = dict(
    type='ImageClassifier',
     backbone=dict(type='SwinTransformerSSR_LoRA',arch='base',img_size=384,stage_cfgs=dict(block_cfgs=dict(window_size=12))),
    head=dict(
        type='LinearClsHead',
        num_classes=2, 
        in_channels=1024,
...
custom_hooks = [
    dict(type='SGPCLoRAHook')
]

load_from = 'work_dirs/swin_base_patch4_window12_384_22kto1k-d59b0d1d.pth'
work_dir = f'work_dirs/swin_ssr-vtab-few-shot/{run_name}'

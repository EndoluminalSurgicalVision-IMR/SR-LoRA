...
ffn_option = 'parallel'
nshot = 10
run_name = f'in21k-vitadaptformer_ffn-{ffn_option}_bs4_lr{lr}_{nshot}-shot_{dataset}'

model = dict(
    type='ImageClassifier',
    backbone=dict(type='VitAdaptFormer',ffn_option=ffn_option),
    head=dict(
...
data = dict(
    samples_per_gpu=4,  # use 2 gpus, total 128
    train=dict(
        ann_file=f'data_backup/MedFMC/{dataset}/{dataset}_{nshot}-shot_train_exp1.txt'),
    val=dict(ann_file=f'data_backup/MedFMC/{dataset}/{dataset}_{nshot}-shot_val_exp1.txt'),
...
work_dir = f'work_dirs/vit_adapterformer/{run_name}'

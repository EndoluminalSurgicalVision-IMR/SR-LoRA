...
class spu_Linear(nn.Linear, LoRALayer):
    # LoRA implemented in a dense layer
    def __init__(
        self, 
        in_features: int, 
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.r > 0 and not self.merged:
            ...
            if self.r > 0:
                ...
                # print(x.shape, lora_A.shape, lora_B.shape)
            return result
        else:
...
class spu_MergedLinear(nn.Linear, LoRALayer):
    # LoRA implemented in a dense layer
    def __init__(
        self, 
        in_features: int, 
        ...
        **kwargs
        ...
        if r > 0 and any(enable_lora):
            ...
            self.nd_lora_A = dynamic(maximum_rank=r * sum(enable_lora))
            self.nd_lora_B = dynamic(maximum_rank=r)
            self.scaling = self.lora_alpha / self.r
            # Freezing the pre-trained weight matrix
            self.weight.requires_grad = False
            # Compute the indices
    ...
    def get_dimension(self):
        assert self.nd_lora_A.get_dimension() == sum(self.enable_lora) * self.nd_lora_B.get_dimension()
        return self.nd_lora_B.get_dimension()

    def get_rank(self):
        assert self.nd_lora_A.get_rank() == sum(self.enable_lora) * self.nd_lora_B.get_rank()
        return self.nd_lora_B.get_rank()

    def set_rank(self, rank, frozen=False):
        self.nd_lora_A.set_rank(sum(self.enable_lora) * (rank+1)-1, frozen=frozen)
        self.nd_lora_B.set_rank(rank+1-1, frozen=frozen)
    ...
    def train(self, mode: bool = True):
        ...
        if mode:
            ...
            if self.merge_weights and not self.merged:
                # Merge the weights and mark it
                if self.r > 0 and any(self.enable_lora):
                    self.nd_lora_B.set_rank(torch.randint(0, self.r+1-1, (1,)).item())
                    self.nd_lora_A.set_rank(sum(self.enable_lora) * (self.nd_lora_B.get_rank()+1)-1)
                    lora_A = self.nd_lora_A(self.lora_A.T).T
                    lora_B = self.nd_lora_B(self.lora_B)
                    delta_w = F.conv1d(
                        lora_A.unsqueeze(0), 
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.r > 0 and any(self.enable_lora) and not self.merged:
            ...
            if self.r > 0 and any(self.enable_lora):
                ...
                self.nd_lora_B.set_rank(torch.randint(0, self.r+1-1, (1,)).item())
                self.nd_lora_A.set_rank(sum(self.enable_lora) * (self.nd_lora_B.get_rank()+1)-1)
                # print(self.nd_lora_A.get_rank(),self.nd_lora_B.get_rank())
                lora_A = self.nd_lora_A(self.lora_A.T).T
                lora_B = self.nd_lora_B(self.lora_B)
                ...
                # print(delta_w.shape,x.shape,self.zero_pad(delta_w).shape)

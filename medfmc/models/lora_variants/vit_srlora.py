...
from medfmc.models.lora_variants.glora import set_glora
...
from .spu import spu_Linear, spu_MergedLinear
...
class MultiheadAttention(BaseModule):
    ...
    def __init__(self,
        ...
        # self.qkv = lora.Linear(self.input_dims, embed_dims * 3, bias=qkv_bias, r=lora_rank[0])
        self.qkv = spu_MergedLinear(self.input_dims, embed_dims * 3, r=lora_rank[0], enable_lora=[True, False, True])
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = spu_Linear(embed_dims, embed_dims, bias=proj_bias, r=lora_rank[1])
...
class VitSR_LoRA(VisionTransformer):
    ...
    #for large mae:
        ...
        for name, param in self.named_parameters():
            if 'lora' in name: #or name in ['cls_token', 'pos_embed', 'patch_embed.projection.weight', 'patch_embed.projection.bias']:
                # import pdb; pdb.set_trace()
                continue
            else:
    ...
    def set_ranks(self, all_ranks, frozen):
        for i, layer in enumerate(self.layers):
            layer.set_ranks(all_ranks[i], frozen=frozen)
        # for layer in self.layers:
        #     layer.set_ranks(ranks, frozen=frozen)

    def get_dimensions(self):
        all_dimensions = []
        for layer in self.layers:
            all_dimensions.append(layer.get_dimensions())
        return all_dimensions

    def get_ranks(self):
        all_ranks = []
        for layer in self.layers:
            all_ranks.append(layer.get_ranks())
        return all_ranks

...
class WindowMSALoRA(BaseModule):
    ...
    def __init__(self,
        ...
        self.proj = psm_Linear(embed_dims, embed_dims, bias=qkv_bias, r=lora_rank[1])
...
class ShiftWindowMSALoRA(BaseModule):
    ...
    def __init__(self,
        ...
        self.qkv = psm_MergedLinear(self.input_dims, embed_dims * 3, r=lora_rank[0], enable_lora=[True, False, True])
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = psm_Linear(embed_dims, embed_dims, bias=True, r=lora_rank[1])
        self.proj_drop = nn.Dropout(proj_drop)
...
class SwinTransformerSSR_LoRA(SwinTransformer):
    ...
    def __init__(
        ...
        self.sgpc_lora_layers = []
        # self.sgpc_acc_empty = 0.0
        self.sgpc_cur_p = 0.0
 

    def init_sgpc_lora(self):
        """initialize sgpc lora layers and masks."""
        self.sgpc_lora_layers = []
        def find_lora_layers(module):
            for child in module.children():
                if hasattr(child, 'r') and getattr(child, 'r', 0) > 0:
                    self.sgpc_lora_layers.append(child)
                    child.set_sgpc_mask(np.zeros(child.r, dtype=np.float32))
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, *args,
        ...
        if (version is None or version < 2) and \
                self.__class__ is SwinTransformerSSR_LoRA:

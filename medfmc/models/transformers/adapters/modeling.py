...
# from transformers.adapters.tensor_buffer import TensorBuffer
...
class Adapter(nn.Module):
    ...
    def __init__(
        ...
        if self.add_layer_norm_before:
            # bottleneck adapter does not use it
            assert 1 == 2
        ...
        if config["phm_layer"]:
            # Linear down projection of the input
            seq_list.append(PHMLayer(adapter_name, self.down_sample, self.input_size, "up", config))
        else:
            seq_list.append(nn.Linear(self.down_sample, self.input_size))
        
        self.adapter_module = nn.ModuleList([nn.Sequential(*seq_list)])
        if config.num_layer > 1:
            tied_layers = [self.adapter_module[0]]
            for _ in range(1, config.num_layer):
                seq_list = []
                if self.add_layer_norm_before:
                    # bottleneck adapter does not use it
                    assert 1 == 2
                ...
                tied_layers.append(nn.Sequential(*seq_list))
            self.adapter_module = nn.ModuleList(tied_layers)
        ...
        if config["init_weights"] == "bert":
            self.adapter_module.apply(self.init_bert_weights)
            if config.num_layer > 1:
                first_layer = list(self.adapter_module[0].parameters())
                other_layers = [list(x.parameters()) for x in self.adapter_module[1:]]

                for layer_idx in range(len(first_layer)):
                    for layer in other_layers:
                        layer[layer_idx].data.copy_(first_layer[layer_idx].data)
    ...
    def forward(self, x, residual_input, output_gating=False):
        up = self.adapter_module[0](x)
        for i in range(1, self.config.num_layer):
            up += self.adapter_module[i](x)
        up = up * self.scaling
        output = up
